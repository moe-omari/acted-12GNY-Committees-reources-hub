...
const content = {
  ...
  ar: {
    ...
    subtitle:
      "مركز موارد متكامل لأعضاء لجنة إدارة الموقع — يضم الوثائق الرسمية والسياسات التشغيلية ونماذج العمل والروابط المرجعية اللازمة لضمان إدارة فعّالة ومستدامة للموقع. ابحث وصنّف حسب الوسوم وحمّل ما تحتاجه بكل سهولة.",
...
function displayTitle(resource: ResourceItem, lang: Language) {
  if (lang === "ar") return resource.titleAr || resource.titleEn;
  return resource.titleEn || resource.titleAr;
}

function displayDescription(resource: ResourceItem, lang: Language) {
  if (lang === "ar") return resource.descriptionAr || resource.descriptionEn;
...
export function ResourceListClient({ initialResources }: { initialResources: ResourceItem[] }) {
  ...
  const searchPlaceholder = isArabic ? "ابحث بالعنوان أو الوصف" : "Search by title or description";
  ...
  const sortedResources = useMemo(
    () => [...initialResources].sort((a, b) => (a.createdAt < b.createdAt ? 1 : a.createdAt > b.createdAt ? -1 : 0)),
    [initialResources],
  );
  ...
  const filteredResources = useMemo(() => {
    ...
    return sortedResources.filter((resource) => {
      const searchableText = [resource.titleEn, resource.titleAr, resource.descriptionEn, resource.descriptionAr]
        .join(" ")
        .toLowerCase();
      const matchesText = !query || searchableText.includes(query);
      const resourceTags = [...resource.tagsEn, ...resource.tagsAr].map((tag) => tag.toLowerCase());
      const matchesTags =
        selectedTags.length === 0 || selectedTags.every((tag) => resourceTags.includes(tag.toLowerCase()));
            ...
            {selectedTags.length > 0 ? (
              <div className="selected-tags-wrap" aria-label={selectedLabel}>
                {selectedTags.map((tag) => (
                  <button key={tag} type="button" className="tag-filter-pill" onClick={() => removeTagFilter(tag)}>
                    {tag} ×
                  </button>
                ))}
              </div>
      ...
      <main className="container resource-grid">
        {filteredResources.length === 0 ? (
          ...
          filteredResources.map((resource) => {
            const href = resource.kind === "url" ? resource.url : `/resources/${resource.id}`;
                ...
                <div className="resource-meta-row">
                  ...
                  {isNew ? (
                    <span className="new-badge" aria-label={copy.new}>
                      <Image src="/glimmer.png" alt="" width={12} height={12} className="glimmer-image" aria-hidden="true" />
                      {copy.new}
                    </span>
                  ) : null}
                </div>
            ...
            if (isExternal) {
              return (
                <a key={resource.id} href={href} target="_blank" rel="noopener noreferrer" className="card-link-wrap" aria-label={title}>
                  {cardBody}
                </a>
              );
            }
